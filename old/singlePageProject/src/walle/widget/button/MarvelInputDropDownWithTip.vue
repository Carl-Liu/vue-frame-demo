<template>
  <div class="inputDropDownBtnWrapper" v-bind:class="[status]">
    <div class="dropdownBtn"  v-click-outside="hideSubMenu"
         v-bind:class="{ disable, dropdownBtnShowBorder: subMenuShow }">
  ...
  export default {
    ...
    methods: {
      init: function (lstItems, strSelectItemLabel) {
        this.items = lstItems.optionItems;
        this.tips = lstItems.tipItems;
        this.inputMsg = strSelectItemLabel;
      ...
      onBlur:function(){
        if(this.subMenuShow){
          ...
          this.isFilter = false;
          this.setOptionFilterStatus();
          this.$emit('onOptionSelect', this.inputMsg);
        }
      },
      onInput:function(){
        this.subMenuShow = true;
