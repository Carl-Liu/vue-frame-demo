...
&lt;marvel-upload-simple btnName="文件上传"
    ...
    <!--2级 end-->
    <!--2级 start-->
    <div class="title level2">多文件上传</div>
    <div class="describe">
      多文件上传
    </div>
    <div class="showArea">
      <marvel-tab :tabItems="tabItems2">
        <marvel-tab-item :isActive="tabItems2[0].isActive">
          <div class="showAreaInner">
            <!--2级DemoView start-->
            <div style="width:200px;">
              <marvel-upload-multi  btnName="文件上传"
                                    @onUploadFile="onUploadFiles"></marvel-upload-multi>
            </div>
            <!--2级DemoView end-->
          </div>
        </marvel-tab-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onUploadFiles: function (lstFiles) {
        console.log(lstFiles);
        var formData = new FormData();
        for(var i = 0; i< lstFiles.length;i++){
          formData.append(lstFiles[i].name, lstFiles[i]);
        }
        this.$http.post('/mvc/upload/test2', formData, {
          headers:{
            "Content-Type" :'multipart/form-data'
